...
\end{description}
	...
	extbf{Kwantitatieve meetwaarden}
\begin{itemize}
  \item \textbf{Bugdetectie:} aantal unieke bugs per tijdsbudget.
  \item \textbf{Time-to-first-bug:} hoe snel de eerste bug gevonden wordt.
  \item \textbf{Testdekking:} gekeken naar de bezochte plekken, checkpoints en unieke gebeurtenissen.
  \item \textbf{False positives:} meldingen van problemen die je later niet kunt reproduceren.
  \item \textbf{Praktische kost:} tijd om alles op te zetten, trainingstijd en compute-gebruik.
\end{itemize}
...
\subsection{Tools}
	extbf{Verwachte stack:} Voor tools gebruiken we Unity, ML-Agents, Python, CSV/JSON, Git en soms OpenCV voor visuele tests.
